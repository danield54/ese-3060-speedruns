...
hyp = {
    'opt': {
        'train_epochs': 9.9,
        'batch_size': int(1024*1.5),         # Increased from 1024 for better GPU utilization (+25% speed)
...
folder = "BatchLR_summary"
os.makedirs(folder, exist_ok=True)
SUMMARY_FILE = os.path.join(folder, "BatchLR_summary_table.csv")
...
def main(run):
    ...
    os.makedirs("expt_logs/BatchLR", exist_ok=True)
    log_filename = f"expt_logs/BatchLR/run_{log_run}.json"
...
if __name__ == "__main__":
    ...
    plt.title("BatchLR Training Time per Epoch")
    plt.grid(True)

    plot_filename_times = "plots/avg_epoch_times_BatchLR.png"
    ...
    plt.title("BatchLR Training Loss at each Epoch")
    plt.grid(True)
    
    plot_filename_losses = "plots/avg_epoch_losses_BatchLR.png"
    ...
    plt.title("BatchLR Validation Accuracy at each Epoch")
    plt.grid(True)
    
    plot_filename_losses = "plots/avg_epoch_valaccs_BatchLR.png"
    plt.savefig(plot_filename_losses, dpi=300, bbox_inches='tight')
    plt.close()
