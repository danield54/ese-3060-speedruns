...
def main(run):
    ...
    os.makedirs("expt_logs/DS_NoLRTuning", exist_ok=True)
    log_filename = f"expt_logs/DS_NoLRTuning/run_{log_run}.json"
...
if __name__ == "__main__":
    ...
    plt.title("DS Training Loss at each Epoch")
    plt.grid(True)
    ...
    plt.title("DS Validation Accuracy at each Epoch")
    plt.grid(True)
    
    plot_filename_losses = "plots/avg_epoch_valaccs_withDS.png"
    plt.savefig(plot_filename_losses, dpi=300, bbox_inches='tight')
    plt.close()
