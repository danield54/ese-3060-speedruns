...
def main(run):
    ...
    model = make_net().float().cuda()
    current_steps = 0

    # Allocate static state buffers for CUDA Graphs
    static_inputs  = torch.empty((batch_size, 3, 32, 32), device='cuda', dtype=torch.float)
    static_labels  = torch.empty((batch_size,), device='cuda', dtype=torch.long)

    # Placeholders for captured outputs/loss
    graph_outputs = None
    graph_loss = None
    ...
    # Define one training step to prepare it to be "captured" by the CUDA Graph
    def graph_train_step():
        nonlocal graph_outputs
        #optimizer.zero_grad()
        graph_outputs = model(static_inputs)
        #graph_loss = loss_fn(graph_outputs, static_labels).sum()
        #graph_loss.backward()
        #optimizer.step()

    # Perform the warmup iterations
    warm_iters = 3
    ...
    for epoch in range(ceil(epochs)):
        ...
        for inputs, labels in train_loader:
            ...
            # Copy batch into static GPU buffers
            static_inputs.copy_(inputs)
            static_labels.copy_(labels)

            # Replay CUDA graph (forward + backward + update + scheduler)
            optimizer.zero_grad()

            with torch.no_grad():
                g.replay()
            graph_loss = loss_fn(graph_outputs.clone(), static_labels).sum()
            graph_loss.backward()
            optimizer.step()
            scheduler.step()
        ...
        train_acc = (graph_outputs.detach().argmax(1) == labels).float().mean().item()
        train_loss = graph_loss.item() / batch_size
