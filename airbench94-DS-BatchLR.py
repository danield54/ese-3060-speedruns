...
hyp = {
    'opt': {
        ...
        'lr': 11.5*1.5,                 # learning rate per 1024 examples
        'momentum': 0.85,
...
folder = "DS_BatchLR_summary" 
os.makedirs(folder, exist_ok=True)
SUMMARY_FILE = os.path.join(folder, "DS_BatchLR_summary_table.csv")
...
# Creating a DepthWiseSepConv
class DepthWiseSepConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding='same', bias=False):
        super().__init__()
        # set in_channels to be both in+out
        self.depthwise = nn.Conv2d(in_channels, in_channels, kernel_size=kernel_size, padding=padding, groups=in_channels, bias=bias)
        self.ptwise = nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=bias)
    ...
    def reset_parameters(self):
        # depthwise conv
        nn.init.kaiming_normal_(self.depthwise.weight, mode='fan_in', nonlinearity='relu')
        if self.depthwise.bias is not None:
            self.depthwise.bias.data.zero_()

        # pointwise conv: mimic Conv initialization
        nn.init.dirac_(self.ptwise.weight.data[:self.ptwise.weight.size(1)])
        if self.ptwise.bias is not None:
            self.ptwise.bias.data.zero_()
...
# Make into custom group
class ConvWithDSGroup(nn.Module):
    def __init__(self, channels_in, channels_out, batchnorm_momentum):
        super().__init__()
        self.conv1 = DepthWiseSepConv(channels_in,  channels_out) # Apply Depthwise-Separable Conv layer
        self.pool = nn.MaxPool2d(2)
        self.norm1 = BatchNorm(channels_out, batchnorm_momentum)
        self.conv2 = DepthWiseSepConv(channels_out, channels_out) # Apply another Depthwise-Separable Conv layer at the very end
        self.norm2 = BatchNorm(channels_out, batchnorm_momentum)
        self.activ = nn.GELU()
...
def make_net():
    ...
    net = nn.Sequential(
        ...
        ConvWithDSGroup(widths['block2'], widths['block3'], batchnorm_momentum),
        nn.MaxPool2d(3),
        Flatten(),
...
def main(run):
    ...
    os.makedirs("expt_logs/DS_BatchLR", exist_ok=True)
    log_filename = f"expt_logs/DS_BatchLR/run_{log_run}.json"
...
if __name__ == "__main__":
    ...
    plt.title("DS BatchLR Training Time per Epoch")
    plt.grid(True)

    plot_filename_times = "plots/avg_epoch_times_withDSBatchLR.png"
    ...
    plt.title("DS BatchLR Training Loss at each Epoch")
    plt.grid(True)
    
    plot_filename_losses = "plots/avg_epoch_losses_withDSBatchLR.png"
    ...
    plt.title("DS BatchLR Validation Accuracy at each Epoch")
    plt.grid(True)
    
    plot_filename_losses = "plots/avg_epoch_valaccs_withDSBatchLR.png"
    plt.savefig(plot_filename_losses, dpi=300, bbox_inches='tight')
    plt.close()
